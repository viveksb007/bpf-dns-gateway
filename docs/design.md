...
| BPF helper failure (`bpf_skb_store_bytes`, `bpf_l3_csum_replace`, `bpf_l4_csum_replace`) | All returns checked via the `nat_rewrite_addr` helper. On a mid-sequence failure the already-applied steps are reverted so the packet leaves either fully rewritten or byte-identical to arrival, then passes through (`nat_errors_total`). Ingress additionally deletes the just-inserted conntrack entry so map state matches the packet. If the revert itself fails (never expected — these helpers only fail on pathological offsets), the packet may be inconsistent and `nat_revert_failures_total` is incremented; alarm on nonzero. |
